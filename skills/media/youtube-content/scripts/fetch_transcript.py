...
def fetch_transcript(video_id: str, languages: list = None):
    ...
    Compatible with youtube-transcript-api v2.x (uses .fetch() instance method).
    The old YouTubeTranscriptApi.get_transcript() no longer exists in v2+.
    """
    try:
    ...
    # v2.x returns FetchedTranscriptSnippet objects with .text/.start/.duration
    return [

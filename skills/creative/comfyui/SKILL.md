...
## Quick Readiness Check (Fastest Path)

When someone asks "is ComfyUI installed/ready/has models?", do NOT run filesystem
commands (`find`, `ls`, `mdfind`) on the ComfyUI directory — these can hang for
minutes on large model directories or slow filesystems. Use the running server
instead:

```bash
# 1. Is the server alive? (<1s)
curl -s --max-time 3 http://127.0.0.1:8188/system_stats

# 2. What checkpoints are available? (<2s)
curl -s --max-time 5 http://127.0.0.1:8188/object_info | python3 -c "
import sys,json
d=json.load(sys.stdin)
for k,v in d.items():
    if 'CheckpointLoader' in v.get('class_type',''):
        ckpts = v['input']['required'].get('ckpt_name',[[]])
        print(f'{k}: {len(ckpts[0])} checkpoints')
        for c in ckpts[0][:5]: print(f'  - {c}')
"

# 3. Where is ComfyUI installed? (<1s)
# Check for the comfy-cli workspace or common paths:
uvx --from comfy-cli comfy --help >/dev/null 2>&1 && echo "comfy-cli: available"
ls -d ~/Documents/comfy/ComfyUI ~/ComfyUI /Applications/ComfyUI.app 2>/dev/null
```

These three commands answer "installed? running? has models?" in under 5 seconds.
...
1. **API format required** — every script and the `/api/prompt` endpoint expect
   API-format workflow JSON. The scripts detect editor format (top-level
   `nodes` and `links` arrays) and tell you to re-export.
...
4. **Missing custom nodes** — use `check_deps.py`.

5. **Working directory** — comfy-cli auto-detects; use `--workspace` if needed.

6. **Cloud free-tier API limits** — 403 on free accounts.

7. **Timeout for video/audio workflows** — auto-detected, default 300s→900s.

8. **Path traversal in output filenames** — protected by `safe_path_join`.

9. **Workflow JSON is arbitrary code** — inspect untrusted workflows.

10. **Auto-randomized seed** — pass `seed: -1` or use `--randomize-seed`.

11. **`tracking` prompt** — use `comfy --skip-prompt tracking disable`.

12. **Python 3.14 asyncio bug** — `comfy launch --background` fails with `RuntimeError: There is no current event loop in thread 'MainThread'`. Workaround: launch ComfyUI directly with `cd ~/Documents/comfy/ComfyUI && .venv/bin/python main.py --listen 127.0.0.1 --port 8188` in background mode.

13. **First launch is slow** — ComfyUI downloads frontend assets and initializes on first run. Allow 30-60 seconds before checking `http://127.0.0.1:8188/system_stats`.

14. **Models directory** — checkpoints go in `~/Documents/comfy/ComfyUI/models/checkpoints/`. SDXL is ~6.5GB download. Use `comfy model download` or direct curl. Verify with `curl http://127.0.0.1:8188/object_info`.

15. **Apple Silicon (MPS) confirmed working** — M1 Max with 64GB unified memory runs SDXL and Flux. No CUDA needed. Verdict from `hardware_check.py`: `ok → apple-silicon`.

16. **uvx-based comfy-cli works** — `uvx --from comfy-cli comfy <command>` avoids needing pipx install. No global pip pollution.

17. **SDXL sampler/scheduler MPS compatibility** — The skill's `sdxl_txt2img.json` workflow (dpmpp_2m/karras at 1024×1024) returns HTTP 500 on macOS MPS. Use euler/normal at 768×768 instead. Proven working template:
```python
{ "3": {"class_type": "KSampler", "inputs": {"seed": SEED, "steps": 20, "cfg": 7,
    "sampler_name": "euler", "scheduler": "normal", "denoise": 1.0, ...}},
  "5": {"class_type": "EmptyLatentImage", "inputs": {"width": 768, "height": 768, ...}},
  ...
}
```
This works reliably on MPS with SDXL at ~45s per 768×768 image. The dpmpp_2m/karras combo at 1024×1024 is the confirmed cause of 500 errors — not a server issue, not a model issue. Do NOT debug; just switch to euler/normal.

18. **Prompt strength matters for backgrounds** — A weak prompt like "on a white background" produces images with plates, fabrics, and gradient backgrounds. Use: `"studio product photo of a single {item}, isolated on pure white seamless background, no surface visible, no plate, no table, no fabric, no props, floating, high key lighting, no shadows on background"`. This produces clean product-shot images matching professional reference guides.
